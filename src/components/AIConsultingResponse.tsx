  ...
  return (
    <section className="response" style={{ borderLeft: `3px solid ${color}` }}>
      ...
      <article className="analysis">
        Analyse<p>{dataResult.analysis}</p>
      </article>
      <article className="recommended-action">
        Action recommandée<p>{dataResult.recommendedAction}</p>
      </article>
      <article className="estimated-impact">
        Impact estimé<p>{dataResult.estimatedImpact}</p>
      </article>
    </section>
  );
}
